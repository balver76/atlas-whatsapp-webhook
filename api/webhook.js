export default async function handler(req, res) {
  ...
  const kvCmd = async (arr) => {
    ...
    if (!resp.ok) throw new Error(`KV error ${resp.status}: ${raw}`);
    return json?.result;
  };

  const kvGet = async (key) => (key ? kvCmd(["GET", String(key)]) : null);
  const kvSet = async (key, value) => {
    if (!key) return null;
    const v = typeof value === "string" ? value : JSON.stringify(value);
    return kvCmd(["SET", String(key), v]);
  };
  const kvDel = async (key) => (key ? kvCmd(["DEL", String(key)]) : null);
  ...
  try {
    ...
    // Status updates (sent/delivered/read) — just log & return OK
    ...
    if (!from) return end(200, "OK");

    // Only handle text
    if (type !== "text") {
      await sendText(from, "Please send a text message to continue. Type 'restart' to start again.");
      return end(200, "OK");
    }
    ...
    let state =
      (await kvGetJson(stateKey)) || {
        step: 0, // 0=menu, 1=pickup, 2=dest, 3=datetime, 4=pax, 5=confirm
        pickup: "",
        dest: "",
        datetime: "",
        pax: "",
    ...
    // Step 0: selection OR smart pickup capture
    if (state.step === 0) {
      if (t === "1") {
        state.step = 1;
        await kvSet(stateKey, state);
      ...
      // Smart: treat any other text as pickup and move forward
      state.pickup = text;
      state.step = 2; // destination next
      await kvSet(stateKey, state);
    ...
    // Step 2: destination -> date/time
    if (state.step === 2) {
      state.dest = text;
      state.step = 3;
      await kvSet(stateKey, state);
      await sendText(from, "✅ Destination saved. Please enter date & time (e.g., 26/01 18:30):");
      return end(200, "OK");
    }
